 ...
 */
function cleanStaleChromelocks(profileDir) {
...
function resolveProfileDir(requestedDir) {
    if (requestedDir) {
        ...
        try {
            ...
            if (lockPid && lockPid !== process.pid) {
                try {
                    ...
                    console.error(`Primary profile locked by PID ${lockPid}; using fallback: ${fallbackDir}`);
                    return fallbackDir;
                }
                catch {
...
async function restartChromeWithCDP(port = 9222) {
    ...
    if (!chromePath) {
        throw new Error("Could not find Chrome installation. Install Google Chrome or pass the path manually.");
    }
    ...
    if (!ready) {
        throw new Error(`Chrome relaunched but CDP endpoint not responding on port ${port} after 15s. ` +
...
export class SessionManager {
     ...
     */
    async connectCDP(cdpUrl = "http://localhost:9222") {
    ...
    async getPage(config) {
        ...
        switch (config.browserMode) {
            ...
            case "attach": {
                if (!config.cdpUrl) {
                    throw new Error("attach mode requires cdp_url parameter. Launch Chrome with:\n" +
     ...
     */
    async exportCookies(domains) {
        if (!this.context) {
            throw new Error("No browser context — call getPage() first to start a session");
        }
        let cookies = await this.context.cookies();
        if (domains && domains.length > 0) {
            cookies = cookies.filter(c => domains.some(d => c.domain.includes(d)));
        }
        return cookies;
    }
    async close() {
    ...
    configMatches(config) {
        if (!this.config)
            return false;
        return (this.config.browserMode === config.browserMode &&
