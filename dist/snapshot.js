 ...
 */
export async function navigateWithFallback(page, url, timeout = 30000) {
    try {
...
export async function takeSnapshot(page, options) {
    ...
    while (retries > 0) {
        try {
            ...
            retries--;
            if (retries === 0)
                throw new Error("Page keeps navigating — unable to take stable snapshot. The page may require authentication or is stuck in a redirect loop.");
    ...
    const rawElements = await page.evaluate(domWalkerScript);
    // Resolve raw elements into SpatialElements
    let elements = rawElements.map((raw, idx) => {
        // Determine role
        ...
        const isActionableAttr = raw.hasOnClick ||
            raw.tabIndex >= 0 ||
    ...
    elements = elements.filter((el) => el.bounds.x >= -100 && el.bounds.y >= -100);
    ...
    for (const el of elements) {
        if (el.parent_idx !== null)
            parentMap.set(el.idx, el);
    }
    ...
    elements = elements.filter((el) => {
        ...
        if (el.bounds.width <= 1 && el.bounds.height <= 1)
            return false;
