...
The full DOM map is always cached server-side regardless of output format. Use spatial_query to search the cached map without re-snapshotting.`, PublicSnapshotInput.shape, async (params) => {
    try {
        ...
        const snapshot = result?.spatial_map ?? result;
...
// ─── spatial_click ───────────────────────────────────────────────
server.tool("spatial_click", `Click an element on the page using REAL OS-level mouse events (cliclick / CGEvent).
...
Returns an updated spatial map reflecting the page state after the click.`, ClickInput.shape, async (params) => {
    try {
        ...
        lastSnapshot = snapshot;
        return {
            content: [{ type: "text", text: formatSnapshot(lastSnapshot, lastMaxElements, lastCompact, lastOutputFormat, lastVerbosity) }],
        };
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
...
Returns an updated spatial map.`, TypeInput.shape, async (params) => {
    try {
...
server.tool("spatial_get_window_geometry", `Return the current Chrome window's screen geometry — used internally by
...
to dispatch their own OS-level events. Returns:
  ...
  - device_pixel_ratio: number`, {}, async () => {
    try {
        const geom = await client.getWindowGeometry();
        return {
            content: [{ type: "text", text: JSON.stringify(geom, null, 2) }],
        };
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error getting window geometry: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_scroll ──────────────────────────────────────────────
server.tool("spatial_scroll", `Scroll the page or a specific scrollable container. Use delta_y with positive values to scroll down, negative to scroll up.

Returns an updated spatial map reflecting the new scroll position.`, ScrollInput.shape, async (params) => {
    try {
...
// ─── spatial_query ───────────────────────────────────────────────
server.tool("spatial_query", `Filter the current spatial map without re-loading the page. Search for elements by ARIA role, HTML tag, label text, bounding box region, or actionability.
...
server.tool("spatial_get_injectable", `Get the NeoVision spatial snapshot as injectable JavaScript — for running in ANY browser context.
...
Anti-bot systems like DataDome don't just look at speed — they look at PATTERNS. This tool adds natural variance: randomized delays, periodic breaks, and automatic slowdown when CAPTCHAs are detected.`, {
    ...
    if (action === "record_success") {
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ status: "recorded" }, null, 2) }],
        };
    }
    if (action === "record_captcha") {
        activePacer.recordCaptcha();
        return {
            content: [{ type: "text", text: JSON.stringify({ status: "captcha_recorded" }, null, 2) }],
        };
    }
    if (action === "record_captcha_solved") {
        activePacer.recordCaptchaSolved();
        return {
            content: [{ type: "text", text: JSON.stringify({ status: "captcha_solved" }, null, 2) }],
        };
    }
    if (action === "status") {
        return {
            content: [{ type: "text", text: JSON.stringify({ status: "active", stats: activePacer.getStats() }, null, 2) }],
        };
    }
    if (action === "estimate") {
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ estimated_time: estimate.totalHuman }, null, 2) }],
        };
    }
    ...
    if (action === "end") {
        ...
        activePacer = null;
        return {
            content: [{ type: "text", text: JSON.stringify({ status: "session_ended", summary: stats }, null, 2) }],
        };
    }
    return {
...
// ─── spatial_screenshot ──────────────────────────────────────────
server.tool("spatial_screenshot", `Take a screenshot of the current page in the real Chrome browser.
...
// ─── spatial_navigate ────────────────────────────────────────────
server.tool("spatial_navigate", `Navigate to a URL without taking a snapshot. Lighter weight than spatial_snapshot.`, {
    ...
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error navigating: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_wait ────────────────────────────────────────────────
server.tool("spatial_wait", `Wait for a specified duration.`, {
    ...
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error waiting: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_execute_js ──────────────────────────────────────────
server.tool("spatial_execute_js", `Execute JavaScript in the page via the Chrome extension.
...
Returns: { result, world } — the expression's value and which world ran it.`, {
    ...
    world: z.enum(["isolated", "main"]).default("isolated").describe("Execution world. 'isolated' (default) bypasses page CSP but can't see page-world vars. 'main' has page-world access but fails on CSP-strict sites."),
}, async (params) => {
    try {
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ result, world: input.world }, null, 2) }],
        };
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error executing JS: ${msg}` }],
            isError: true,
        };
    }
});
