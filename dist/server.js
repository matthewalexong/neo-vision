...
import { z } from "zod";
import { PublicSnapshotInput, ClickInput, TypeInput, ScrollInput, QueryInput, ImportCookiesInput, ExportCookiesInput, ConnectCDPInput, DisconnectCDPInput } from "./schema.js";
...
let lastSnapshotOptions = null;
let lastSessionConfig = null;
let lastMaxElements = 2000;
let lastCompact = true;
let lastOutputFormat = "compact";
 ...
 */
function compactElement(el, verbosity = "actionable") {
    const base = {
        idx: el.idx,
        tag: el.tag,
        role: el.role,
 ...
 */
function formatAgentSnapshot(snapshot, maxElements) {
    ...
    const viewportTextCandidates = snapshot.elements.filter((el) => inViewport(el) && el.text != null && el.text.trim().length > 0);
    ...
    const actionable = snapshot.elements.filter((el) => el.actionable && el.click_center && inViewport(el));
...
function formatSnapshot(snapshot, maxElements, compact, outputFormat = "compact", verbosity = "actionable") {
...
// ─── spatial_snapshot ────────────────────────────────────────────
server.tool("spatial_snapshot", `Navigate to a URL and return a spatial map of the page. Launches a real Chrome browser automatically — no setup required.
...
The browser session persists across calls. Calling this again with a different URL navigates to that URL. Calling with the same URL reloads it.`, PublicSnapshotInput.shape, async (params) => {
    try {
        ...
        const json = formatSnapshot(lastSnapshot, input.max_elements, input.compact, input.output_format, input.verbosity);
        return {
            content: [
                {
                    type: "text",
        ...
        return {
            content: [{ type: "text", text: `Error taking snapshot: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_click ───────────────────────────────────────────────
server.tool("spatial_click", `Click an element on the page at exact pixel coordinates. Use the click_center.x and click_center.y values from a spatial_snapshot element.
...
Requires an active session — call spatial_snapshot first to open a page.`, ClickInput.shape, async (params) => {
    try {
        const input = ClickInput.parse(params);
        const page = await getActivePage();
        if (!lastSnapshotOptions) {
            return {
                content: [{ type: "text", text: "Error: No snapshot taken yet. Call spatial_snapshot first." }],
                isError: true,
            };
        }
        lastSnapshot = await click(page, input.x, input.y, input.button, input.click_count, lastSnapshotOptions);
        return {
            content: [{ type: "text", text: formatSnapshot(lastSnapshot, lastMaxElements, lastCompact, lastOutputFormat, lastSnapshotOptions?.verbosity ?? "actionable") }],
        };
    }
    catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
...
// ─── spatial_type ────────────────────────────────────────────────
server.tool("spatial_type", `Type text into an element. If x/y coordinates are provided, clicks that position first to focus the element, then types. Otherwise types into whatever element currently has focus.
...
Returns an updated spatial map. Requires an active session — call spatial_snapshot first.`, TypeInput.shape, async (params) => {
    try {
        ...
        lastSnapshot = await typeAction(page, input.text, {
            x: input.x,
            y: input.y,
...
// ─── spatial_scroll ──────────────────────────────────────────────
server.tool("spatial_scroll", `Scroll the page or a specific scrollable container. Use delta_y with positive values to scroll down, negative to scroll up. Optionally target a specific scrollable element by passing its x/y coordinates.

Returns an updated spatial map reflecting the new scroll position and any newly visible elements. Requires an active session — call spatial_snapshot first.`, ScrollInput.shape, async (params) => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error scrolling: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_query ───────────────────────────────────────────────
server.tool("spatial_query", `Filter the current spatial map without re-loading the page. Search for elements by ARIA role, HTML tag, label text, bounding box region, or actionability. Much faster than taking a new snapshot — use this when the page hasn't changed and you need to find specific elements.

Requires a prior spatial_snapshot call (uses the cached map).`, QueryInput.shape, async (params) => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error querying: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_import_cookies ─────────────────────────────────────
server.tool("spatial_import_cookies", `Import cookies into NeoVision's browser session. Use this to warm up the session with cookies from the user's real browser, allowing NeoVision to bypass anti-bot systems (DataDome, Cloudflare, PerimeterX) that block fresh browser profiles.
...
Requires an active session — call spatial_snapshot first to open a page (any page, even about:blank).`, ImportCookiesInput.shape, async (params) => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error importing cookies: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_export_cookies ─────────────────────────────────────
server.tool("spatial_export_cookies", `Export cookies from NeoVision's current browser session. Useful for saving session state or debugging cookie issues. Optionally filter by domain.

Requires an active session — call spatial_snapshot first.`, ExportCookiesInput.shape, async (params) => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error exporting cookies: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_connect_cdp ───────────────────────────────────────
server.tool("spatial_connect_cdp", `Connect NeoVision to the user's real Chrome browser via Chrome DevTools Protocol (CDP).
...
Call spatial_disconnect_cdp when done to release the connection (the user's Chrome stays open).`, ConnectCDPInput.shape, async (params) => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error connecting to CDP: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_disconnect_cdp ────────────────────────────────────
server.tool("spatial_disconnect_cdp", `Disconnect NeoVision from the user's Chrome. The browser stays open — this just releases Playwright's CDP handle. After disconnecting, spatial_snapshot will launch its own stealth Chrome again.`, DisconnectCDPInput.shape, async () => {
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: "text", text: `Error disconnecting: ${msg}` }],
            isError: true,
        };
    }
});
// ─── spatial_get_injectable ──────────────────────────────────────
server.tool("spatial_get_injectable", `Get the NeoVision spatial snapshot as injectable JavaScript — for running in ANY browser context without Playwright.
...
**Works in:** Claude in Chrome javascript_tool, Chrome extensions, Playwright page.evaluate(), DevTools console, bookmarklets.`, {
    ...
    const opts = {
        ...
        verbosity: args.verbosity || "actionable",
    };
    let js;
    let description;
    if (mode === "installer") {
...
server.tool("spatial_pace", `Manage human-like pacing for multi-page scraping sessions.
...
server.tool("bridge_navigate", `Navigate the real Chrome browser to a URL via the NeoVision extension.
...
server.tool("bridge_execute_js", `Execute JavaScript in the page context of the real Chrome browser via the NeoVision extension.
...
server.tool("bridge_inject_spatial", `Inject the NeoVision spatial snapshot into the real Chrome browser via the extension.
...
server.tool("bridge_click", `Click at pixel coordinates in the real Chrome browser via the NeoVision extension.
...
server.tool("bridge_type", `Type text into an element at pixel coordinates in the real Chrome browser.
...
server.tool("bridge_scroll", `Scroll the page in the real Chrome browser.`, {
...
server.tool("bridge_wait", `Wait for a specified duration. Use for pacing between page navigations.`, {
...
server.tool("bridge_screenshot", `Take a screenshot of the current page in the real Chrome browser.
...
server.tool("bridge_get_page_info", `Get info about the current page in the real Chrome browser (URL, title, tab ID).`, {
    tab_id: z.number().optional().describe("Target tab ID (optional)"),
}, async (args) => {
    try {
...
server.tool("bridge_get_page_text", `Extract the text content from the current page in the real Chrome browser.
